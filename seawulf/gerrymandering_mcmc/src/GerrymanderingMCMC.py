...
class GerrymanderingMCMC():
    ...
    def __winning_party_for_district(self, graph, district_label):
        ...
        demo_count = reduce(lambda demo_count, n_label: demo_count +
                            1 if district.nodes[n_label]["voting_history"] == "D" else demo_count - 1, district.nodes, 0)
        if demo_count == 0:
            return None
        elif demo_count < 0:
            return "R"
    ...
    def plot_data(self):
        plt.hist([d["eg"] for d in self.data],
                 bins="auto", alpha=0.5, facecolor='blue')
        ...
        plt.clf()
        plt.hist([d["d_districts"] for d in self.data], bins=5,
                 range=(0, 4), alpha=0.5, facecolor='blue')
        ...
        plt.clf()
        plt.hist([d["r_districts"] for d in self.data], bins=5,
                 range=(0, 4), alpha=0.5, facecolor='blue')
    ...
    def generate_alternative_plans(self, rounds):
        ...
        for i in range(0, self.cooling_period):
            print("Randomizing the seed plan",
                  i) if i % 25 == 0 and self.verbose else None
        ...
        for i in range(0, rounds):
            print("Finding recomb ... ",
                  i) if i % 20 == 0 and self.verbose else None
